...
-- Environment / Executor Helpers Safety Wrappers
local genv = (type(getgenv) == "function" and getgenv()) or _G
local fSignal = (type(firesignal) == "function" and firesignal) 
             or (type(fire_signal) == "function" and fire_signal) 
             or (type(fireSignal) == "function" and fireSignal)
             or (genv and (genv.firesignal or genv.fire_signal or genv.fireSig))

local fPrompt = (type(fireproximityprompt) == "function" and fireproximityprompt)
             or (genv and genv.fireproximityprompt)

local getConn = (type(getconnections) == "function" and getconnections)
             or (genv and genv.getconnections)

-- Fetch Game Name via MarketplaceService Async
...
--------------------------------------------------------------------
-- Visualizers & Safe Protection Spheres
--------------------------------------------------------------------
    ...
    task.spawn(function()
        for i = 1, 4 do
            pcall(function()
                ...
                if continueBtn then
                    if continueBtn:IsA("GuiButton") and getConn then
                        pcall(function()
                            for _, conn in ipairs(getConn(continueBtn.MouseButton1Click)) do
                                if conn and conn.Fire then conn:Fire() end
                            end
                        end)
                    end
    ...
    task.spawn(function()
        pcall(function()
            ...
            if part3 then
                local char = plr.Character
                local root = char and (char:FindFirstChild("HumanoidRootPart") or char.PrimaryPart)
                if root then root.CFrame = part3.CFrame + Vector3.new(0, 3, 0) end
                ...
                while (os.clock() - startTime) < 2 do
                    ...
                    if closestPrompt and fPrompt then
                        pcall(function() fPrompt(closestPrompt) end)
                    end
        ...
        if #outputSections > 0 then
            ...
            if type(writefile) == "function" and type(appendfile) == "function" then
                pcall(function()
                    ...
                    if not hasIsFile or not isfile("SelectAttributesLog.txt") then
...
if isSupportedPlace then
    ...
    GiftGroup:AddToggle("AutoStartCollectingToggle", {
        ...
        Callback = function(Value)
            ...
            if Value then
                ...
                if newLevelSound and newLevelSound:IsA("Sound") then
                    if connections["AutoStartCollectingConn"] then pcall(function() connections["AutoStartCollectingConn"]:Disconnect() end) end
    ...
    FarmGroup:AddDropdown("EnemiesWhitelistDropdown", {
        ...
        Callback = function(Value) Settings.EnemiesWhitelist = Value end
    })
    ...
    FarmGroup:AddDropdown("CursesWhitelistDropdown", {
        ...
        Callback = function(Value) Settings.CursesWhitelist = Value end
    })
    ...
    FarmGroup:AddDropdown("UpgradesWhitelistDropdown", {
        ...
        Callback = function(Value) Settings.UpgradesWhitelist = Value end
    })
    ...
    PlayerGroup:AddSlider("GiftCollectionRange", {
        ...
        Callback = function(Value)
            local magnetAmount = (Value == 0) and 1 or (Value * 30)
    ...
    local function setupUpgradeGuardian(intv, name)
        if upgradeValueGuards[name] then pcall(function() upgradeValueGuards[name]:Disconnect() end); upgradeValueGuards[name] = nil end
...
SystemGroup:AddButton({
    ...
    Func = function()
        ...
        _G.NullGui_Cleanup()
        if consoleConnection then pcall(function() consoleConnection:Disconnect() end) end
        if persistentRespawnConnection then pcall(function() persistentRespawnConnection:Disconnect() end) end
